...
/**
 * Get current 30-second game period information
 */
export const getCurrentPeriod = async (
  ...
  try {
    const period = await thirtySecondGameService.getCurrentPeriod();
    ...
    console.error("[30s] Error fetching current period:", error);
    res.status(500).json({
      success: false,
...
export const placeBet = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const bet = await thirtySecondGameService.placeBet(
      userId,
    ...
    console.error("[30s] Error placing bet:", error);
...
export const getUserBetsForCurrentPeriod = async (
  ...
  try {
    ...
    const currentPeriod = await thirtySecondGameService.getCurrentPeriod();
    if (!currentPeriod) {
      return res.status(404).json({
        success: false,
        message: "No active period found",
    ...
    const bets = await thirtySecondGameService.getUserBetsForPeriod(
      userId,
    ...
    console.error("[30s] Error fetching user bets:", error);
    res.status(500).json({
      success: false,
...
export const getUserBetHistory = async (
  ...
  try {
    ...
    const { items: bets, total } = await thirtySecondGameService.getUserBetHistory(
      userId,
      limit,
      offset
    );
    ...
    console.error("[30s] Error fetching bet history:", error);
    res.status(500).json({
      success: false,
...
export const getPeriodHistory = async (
  ...
  try {
    ...
    const { items: periods, total } = await thirtySecondGameService.getPeriodHistory(
      limit,
      offset
    );
    ...
    console.error("[30s] Error fetching period history:", error);
    res.status(500).json({
      success: false,
...
export const getGameSettings = async (
  ...
  try {
    const settings = await thirtySecondGameService.getGameSettings();
    ...
    console.error("[30s] Error fetching game settings:", error);
    res.status(500).json({
      success: false,
