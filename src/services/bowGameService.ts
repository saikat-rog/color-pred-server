...
export class BowGameService {
  ...
  private async ensureGameSettings() {
    const settings = await prisma.bowGameSettings.findFirst();
    if (!settings) {
      await prisma.bowGameSettings.create({
        data: {
          periodDuration: 300, // 5 minutes (different from main game)
          bettingDuration: 270, // 4:30 minutes
          winMultiplier: 1.8,
          minBetAmount: 10,
          maxBetAmount: 10000,
  ...
  private async startOrResumePeriod() {
    ...
    const periodDurationMinutes = settings.periodDuration / 60;
  ...
  async placeBet(
    ...
    console.log(`🎲 [BOW] User ${userId} placed ${betDescription} of ${amount}`);
      ...
      for (const commission of commissions) {
        ...
        if (commissionAmount > 0) {
          ...
          if (referrer) {
            ...
            await prisma.bowReferralCommission.create({
              data: {
  ...
  async getGameSettings() {
    return await prisma.bowGameSettings.findFirst();
  }
  ...
  async updateGameSettings(data: {
    ...
    return await prisma.bowGameSettings.update({
