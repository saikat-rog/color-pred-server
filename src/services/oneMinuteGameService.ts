...
export class OneMinuteGameService {
  private currentPeriod: any = null;
  private periodTimer: NodeJS.Timeout | null = null;
  private midnightTimer: NodeJS.Timeout | null = null;

  /**
   * Initialize the 1-minute game service and start the period cycle
   */
  async initialize() {
    console.log("🕐 Initializing One Minute Game Service...");
  ...
  private scheduleMidnightTask() {
    ...
    console.log(
      `⏰ [1m] Scheduling next day period generation in ${Math.round(
        timeUntilGeneration / 1000 / 60
      )} minutes (at 23:59:00)`
    );

    this.midnightTimer = setTimeout(async () => {
      console.log("🌙 [1m] 23:59:00 reached - generating tomorrow's periods");
  ...
  private async generatePeriodsForTomorrow() {
    ...
    const periodDurationSeconds = settings.oneMinutePeriodDuration;
    const periodDurationMs = periodDurationSeconds * 1000;
    const bettingDurationMs = settings.oneMinuteBettingDuration * 1000;
    ...
    // Calculate total periods in a day (1440 periods for 1-minute intervals)
    const totalPeriodsInDay = Math.floor(
      (24 * 60 * 60) / periodDurationSeconds
    );

    // Fetch all existing periods for tomorrow in ONE query
    const existingPeriods = await prisma.oneMinuteGamePeriod.findMany({
      where: {
        startTime: {
          gte: tomorrowStart,
          lte: tomorrowEnd,
    ...
    if (periodsToCreate.length > 0) {
      await prisma.oneMinuteGamePeriod.createMany({
        data: periodsToCreate,
        skipDuplicates: true,
      });
      console.log(
        `📅 [1m] Pre-generated ${periodsToCreate.length} periods for tomorrow`
      );
    } else {
      console.log("✅ [1m] All periods for tomorrow already exist");
    }
  }
  ...
  private async generatePeriodsForToday() {
    ...
    if (periodsToCreate.length > 0) {
      ...
      console.log(
        `📅 [1m] Pre-generated ${periodsToCreate.length} periods for today`
      );
    } else {
      console.log("✅ [1m] All periods for today already exist");
    }
  }
  ...
  private async ensureGameSettings() {
    ...
    if (!settings) {
      await prisma.gameSettings.create({
        data: {
          oneMinutePeriodDuration: 60, // 60 seconds (1 minute)
          oneMinuteBettingDuration: 55, // 55 seconds
          winMultiplier: 1.8,
          minBetAmount: 10,
          maxBetAmount: 10000,
  ...
  /**
   * Generate period ID in format YYYYMMDD0001 (4 digits for 1-minute periods - up to 1440 per day)
   */
  ...
  private async startOrResumePeriod() {
    ...
    let period = await prisma.oneMinuteGamePeriod.findUnique({
      where: { periodId },
    });
    ...
    if (!period) {
      console.warn(
        `⚠️ [1m] Period ${periodId} not pre-generated. Creating on-demand...`
      );
      const periodDurationMs = periodDurationSeconds * 1000;
      const bettingDurationMs = settings.oneMinuteBettingDuration * 1000;
      ...
      period = await prisma.oneMinuteGamePeriod.create({
        data: {
          periodId,
          startTime: periodStart,
          endTime,
      ...
      console.log("🆘 [1m] Created period on-demand:", periodId);
    }

    // If period is not_started, activate it
    if (period.status === "not_started") {
      period = await prisma.oneMinuteGamePeriod.update({
        where: { periodId },
        data: { status: "active" },
      });
      console.log("🆕 [1m] Activated period:", periodId);
    } else if (period.status === "active") {
      console.log("♻️ [1m] Resumed active period:", periodId);
    } else if (period.status === "betting_closed") {
      console.log("🔒 [1m] Resumed betting-closed period:", periodId);
    } else {
      console.log("✅ [1m] Period already completed:", periodId);
    }
  ...
  private schedulePeriodEnd(delay: number) {
    ...
    console.log(
      `⏰ [1m] Period will end in ${Math.round(delay / 1000)} seconds`
    );
  }
  ...
  private async lockBetting() {
    ...
    await prisma.oneMinuteGamePeriod.update({
      where: { id: this.currentPeriod.id },
      data: { status: "betting_closed" },
    });

    console.log(
      `🔒 [1m] Betting locked for period: ${this.currentPeriod.periodId}`
    );
  }
  ...
  private async completePeriod() {
    ...
    console.log(`🏁 [1m] Completing period: ${this.currentPeriod.periodId}`);

    // Get period with latest color totals from DB
    const period = await prisma.oneMinuteGamePeriod.findUnique({
      where: { id: this.currentPeriod.id },
    });
    ...
    console.log(
      `🏆 [1m] Period ${period.periodId} completed. Winning color: ${winningColor}, Number: ${winningNumber}`
    );
  ...
  async placeBet(
    ...
    let betDescription = "";
    if (color) {
      betDescription = `1m Bet on color ${color} - Period ${period.periodId}`;
    }
    if (number) {
      betDescription = `1m Bet on number ${number} - Period ${period.periodId}`;
    }
    if (bigOrSmall) {
      betDescription = `1m Bet on ${bigOrSmall} - Period ${period.periodId}`;
    }
    ...
    // Create bet record
    const bet = await prisma.oneMinuteBet.create({
      data: {
        userId,
    ...
    console.log(`🎲 [1m] User ${userId} placed ${betDescription} of ${amount}`);
  ...
  private async processReferralCommissions(
    ...
    try {
          ...
          if (referrer) {
            ...
            await prisma.transaction.create({
              data: {
                ...
                amount: commissionAmount,
                status: "completed",
                description: `L${commission.level} 1m Referral commission from bet #${betId}`,
      ...
      console.error("[1m] Error processing referral commissions:", error);
    }
  }
  ...
  async getCurrentPeriod() {
    ...
    if (this.currentPeriod) {
      try {
        ...
        console.warn("[1m] Warning reading in-memory currentPeriod:", err);
      }
    }

    let period = await prisma.oneMinuteGamePeriod.findFirst({
      where: {
        startTime: { lte: now },
        endTime: { gt: now },
      },
  ...
  async getUserBetsForPeriod(userId: number, periodId: string) {
    return await prisma.oneMinuteBet.findMany({
      where: {
        userId,
        periodId,
      },
  ...
  async getUserBetHistory(
    ...
    const [items, total] = await Promise.all([
      ...
      prisma.oneMinuteBet.count({ where: { userId } }),
    ]);
  ...
  async getPeriodHistory(limit: number = 50, offset: number = 0) {
    const [items, total] = await Promise.all([
      prisma.oneMinuteGamePeriod.findMany({
        where: {
          status: "completed",
        },
        orderBy: {
      ...
      prisma.oneMinuteGamePeriod.count({ where: { status: "completed" } }),
    ]);
...
export const oneMinuteGameService = new OneMinuteGameService();
